...
agent = initialize_agent(llm=llm, agent=agentType, tools=toolkit.get_tools(), verbose=False)  # output logs

hiplot_preprocess_pipeline = pipe.input("query").map("query", "result", lambda query: agent.run(query)).output("result")
