...
class DrawChart:
    ...
    def move_file(self):
        ...
        new_config_path = shutil.copy(os.path.abspath(f"{self.session_path}/data.json"), os.path.join(input_path, f"data.json"))
...
class ChooseChart:
    ...
    def get_module_and_plugin_name(self):
        ...
        print(documents)
        resp = self.json_llm.invoke(get_plugin_name.format(image_description=self.image_description, documents=documents)).content
        print(resp)
        if resp != "None":
    ...
    def move_file(self):
        input_path = os.path.join(self.destination_path, self.tid)
        os.makedirs(input_path, exist_ok=True)
        new_data_path = shutil.copy(os.path.abspath(f"{self.session_path}/data.txt"), os.path.join(input_path, "data.txt"))
        new_config_path = shutil.copy(os.path.abspath(f"{self.session_path}/data.json"), os.path.join(input_path, "data.json"))
        # os.remove("../data.txt")
        # os.remove("../data.json")
    ...
    def get_description(self,text_embedding, img_embedding) -> str:
        ...
        result = custom_collection.combined_search(
...
class ReadChart:
    ...
    def extract_caption(self, path):
        # 调用Java模块并运行命令
        process = subprocess.Popen(["java", "-jar", "/home/lujunjie16/pdffigures2/pdffigures2.jar", path, "-g", f"{self.session_path}/temp.json", "-m", "temp"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "image_path": {
                            "type": "string",
                            "description": "输入图像文件的路径。例如：/path/to/image.jpg"
                        },
                        "module_name": {
                            "type": "string",
                            "description": "要使用的模块名称。例如：basic"
                        },
                        "plugin_name": {
                            "type": "string",
